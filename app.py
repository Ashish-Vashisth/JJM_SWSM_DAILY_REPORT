...
def_columns(df: pd.DataFrame) -> dict:
    return {c: re.sub(r"\s+", "", str(c)).strip().lower() for c in df.columns}
...
def build_report(df: pd.DataFrame, threshold: float = 75.0):
    ...
    scheme_name_col = find_col_contains(norm")
...
def build_report(df: pd.DataFrame, threshold: float = 75.0):
    ...
    if yest_prod_col is None:
        raise KeyError("Could not find 'OHT Water Supply (Meter3Could not find 'OHT Water Supply (Meter3) Yesterday' column Water Supply (Meter3) Yesterday' column.")
...
in norm.items():
    ...
    for c in ["Daily Water Demand (m^3)",r3) Yesterday' column.")
...
def apply_formatting(xlsx_bytes: bytes) -> bytes:
    ...
    def format_sheet(ws):
        ...
        # Body + compute widths
        maxlen = {}
        for r in ws.iter_rows(min_row=2, max_row=ws.max_row, min_col=1ader_fill = PatternFill("solid", fgColor="5B9BD5")
  ...
  header_val = str(ws.cell(row=1, column=c).value or "")
            ...
            ws.column_dimensions[get_column_letter(c)].width = max(10, min(60, int(width * 1.2) + 2h = max(10, min(60, int(width * 1.2) + 2))
...
uploaded = st.file_uploader("Upload JJMUP file", type=["xls",r_input("Threshold (%) for LESS THAN list", min_value=1.0, max_value=100.0, value=75.0, step=1.0)
...
if uploaded:
    ...
    if st.button("Generate Report", type="primary"):
        try:
           ...
           t.file_uploader("Upload JJMUP file", type=["xls", "xlsx", "xlsm"])
...
lication/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            )
   ...
   r("Error while generating report. Please check the uploaded file format/columns.")
            st.exception(e)
else:
